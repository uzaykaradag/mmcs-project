...
def solve_initial_model(transactions, limits_category, limits_description, budget_daily):
    ...
    probability_combined = 0.5 * transactions["prob_customer"] + 0.3 * transactions["prob_description"] + 0.2 * transactions["prob_transaction"]
    ...
    team_assignment = LpVariable.dicts("TeamAssignment", [(bank, trans) for bank in SIZE_INVESTIGATION_TEAM.keys() for trans in transactions.index], cat="Binary")

    # Objective function
    model += lpSum([amount[trans] * probability_combined[trans] * (internal_investigation[trans] + external_investigation[trans]) - cost[trans] * external_investigation[trans] for trans in transactions.index])

    # Constraints
    for trans in transactions.index:
        model += internal_investigation[trans] + external_investigation[trans] <= 1

    for bank in SIZE_INVESTIGATION_TEAM:
        model += lpSum([time_investigation[trans] * team_assignment[(bank, trans)] for trans in transactions.index]) <= SIZE_INVESTIGATION_TEAM[bank]

    for trans in transactions.index:
        model += lpSum([team_assignment[(bank, trans)] for bank in SIZE_INVESTIGATION_TEAM]) == internal_investigation[trans]
    ...
    for category in limits_category:
        ...
        model += lpSum([internal_investigation[trans] + external_investigation[trans] for trans in trans_category]) <= limits_category[category] * lpSum([internal_investigation[trans] + external_investigation[trans] for trans in transactions.index])
    ...
    for description in limits_description:
        ...
        model += lpSum([internal_investigation[trans] + external_investigation[trans] for trans in trans_description]) <= limits_description[description] * lpSum([internal_investigation[trans] + external_investigation[trans] for trans in transactions.index])
...
def solve_second_model(transactions, fraudulent_transactions, budget_daily):
    ...
    team_assign = LpVariable.dicts("TeamAssign", [(bank, trans) for bank in SIZE_INVESTIGATION_TEAM.keys() for trans in transactions.index], cat="Binary")

    # Objective function (focus on fraudulent transactions)
    model += lpSum([amount[trans] * (internal_investigate[trans] + external_investigate[trans]) - cost[trans] * external_investigate[trans] for trans in transactions.index if trans in fraudulent_transactions])
    ...
    for bank in SIZE_INVESTIGATION_TEAM:
        model += lpSum([time_investigation[trans] * team_assign[(bank, trans)] for trans in transactions.index]) <= SIZE_INVESTIGATION_TEAM[bank]
